...
window.runCadScript = async (fileName, bytes, script, options = {}) => {
  ...
  if (hasDrawing) {
    ...
    const opened = await docManager.openDocument(fileName!, buffer, {
      ...
      // Headless exports get no benefit from incremental paints; progressive
      // rendering only adds per-second full-scene repaints during load (very
      // slow under SwiftShader) while yielding conversion time.
