...
export class AcTrBaseView {
  ...
  /**
   * Fits the orthographic near/far planes to a world-space Z range so content
   * far from the default camera plane is not clipped.
   *
   * The default range (near = 0.1, far = 1000, camera z = 500) only covers a
   * ~1500-unit world-Z band; drawings with large elevation spans (for example
   * projected coordinates reaching 1.97e7) would otherwise lose every entity
   * outside that band. When content sits above the camera, the camera is
   * elevated so the whole span stays in front of the near plane.
   *
   * @param zMin - Lowest world-space Z value to keep visible.
   * @param zMax - Highest world-space Z value to keep visible.
   */
  updateCameraDepthRange(zMin: number, zMax: number) {
    if (!Number.isFinite(zMin) || !Number.isFinite(zMax) || zMin > zMax) {
      return
    }
    const margin = 1000
    const camZ = this._camera.position.z
    const elevatedZ = zMax + margin > camZ ? zMax + margin : camZ
    const near = Math.max(0.01, elevatedZ - zMax - margin)
    const far = Math.max(near + 1, elevatedZ - zMin + margin)
    this._camera.position.z = elevatedZ
