...
function toVector2(
  ...
  return points
    ...
    .map(point =>
      offset
        ? new THREE.Vector2(point.x - offset.x, point.y - offset.y)
...
/**
 * Computes the WCS bounding-box center of all boundary loops in double
...
function resolveBoundaryAnchor(
  ...
  for (const loop of pointBoundaries) {
...
export class AcTrPolygon extends AcTrEntity {
  ...
  constructor(
    ...
    // Rebase the triangulation origin to the WCS boundary center (computed in
    // double precision) so ShapeGeometry never writes world-scale coordinates
    // into its float32 position attribute.
    const anchor = resolveBoundaryAnchor(pointBoundaries)

    const geometries: THREE.BufferGeometry[] = []
    this.buildHatchGeometry(pointBoundaries, hierarchy, anchor, geometries)
  ...
  private buildHatchGeometry(
    ...
    noHoles.forEach(index => {
      ...
      const shape = new THREE.Shape(toVector2(points, anchor))
    ...
    for (const pair of holes) {
      ...
      const shape = new THREE.Shape(toVector2(outerPoints, anchor))
      // merge holes
      let mergedHoles: {
        regions: number[][][]
        inverted: boolean
            ...
            if (ix === 0) {
              ...
              if (mergedHoles.regions.length > 0) {
                mergedHoles.regions.forEach((ps: number[][]) => {
                  ...
                  const vec2s = ps.map(
                    (p: number[]) =>
                      new THREE.Vector2(p[0] - anchor.x, p[1] - anchor.y)
                  )
      ...
      for (let i = 0; i < pair[1].length; i++) {
        ...
        if (!ignoreHoleIndexArr.includes(idx)) {
          ...
          shape.holes.push(new THREE.Path(toVector2(holePoints, anchor)))
        }
      }
      createGeometry(shape)
    }
    ...
    node.children.forEach(child => {
      child.children.forEach(grandchild => {
        this.buildHatchGeometry(pointBoundaries, grandchild, anchor, geometries)
      })
    })
  }
