...
export class AcTrLayout {
   ...
   *
   * @param meta - Direct-batch payload to append.
   * @param extendBbox - Whether the entity extends the layout extents.
   * @param spatialIndexBox - Explicit spatial-index box for the entity; pass
   *   `null` to skip registration (used when several metas represent one
   *   entity and only the first registers the union box).
   * @returns `true` when geometry and spatial index were registered.
   */
  addDirectEntity(
    meta: AcTrDirectEntityMeta,
    extendBbox: boolean = true,
    spatialIndexBox?: THREE.Box3 | null
  ): boolean {
    if (!meta.objectId) {
      throw new Error('Object id is required to add one entity!')
    }
    ...
    this.invalidateBox()
    if (spatialIndexBox !== null) {
      this.registerSpatialIndexBox(
        meta.objectId,
        spatialIndexBox ?? meta.wcsBbox
      )
    }
    return true
  }
