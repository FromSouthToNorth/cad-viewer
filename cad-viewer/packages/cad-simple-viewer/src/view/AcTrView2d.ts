...
export class AcTrView2d extends AcEdBaseView {
  ...
  /** Total entities queued since the last clear, for progress reporting */
  private _totalEntitiesToProcess: number
  ...
  /**
   * Minimum interval between automatic camera depth-range checks (ms).
   *
   * The scene Z extent can outgrow the camera's near/far planes after the
   * first framing (Saved-mode opens frame from an early partial box, and
   * layer-driven conversions add geometry afterwards). This throttles the
   * self-healing check in the animate loop; each check is cheap because
   * {@link AcTrScene.box} is cached per layout unless geometry changed.
   */
  private static readonly DEPTH_RANGE_AUTO_REFRESH_INTERVAL_MS = 500

  /** Z extent last applied to the camera by {@link updateCameraDepthRangeFromScene}. */
  private _appliedDepthRangeZ: { min: number; max: number } | null = null

  /** Timestamp of the last automatic depth-range check. */
  private _lastDepthRangeAutoCheckAt = 0
  ...
  constructor(options: AcTrView2dOptions = DEFAULT_VIEW_2D_OPTIONS) {
    ...
    this._totalEntitiesToProcess = 0
  ...
  /**
   * True while open-related work is still pending: entity batch conversion,
   * deferred glyph geometry, or the progressive camera fit that runs until the
   * final zoom-to-fit. The open-file progress overlay stays visible as long as
   * this reports true.
   */
  get isOpenFileWorkPending() {
    return this.isProcessingEntities || this._progressiveOpenFit.isActive
  }

  /**
   * Fraction (0-1) of queued entities already converted into the scene since
   * the last {@link clear}, or `undefined` when nothing is pending.
   *
   * Used by the open-file progress overlay to keep reporting progress while
   * the scene drains after the converter's ENTITY stage has ended.
   */
  get entityProcessingProgress(): number | undefined {
    if (
      this._numOfEntitiesToProcess === 0 ||
      this._totalEntitiesToProcess === 0
    ) {
    ...
    return Math.min(
      ...
      Math.max(
        0,
        1 - this._numOfEntitiesToProcess / this._totalEntitiesToProcess
      )
    )
  }
  ...
  async ensureEntitiesConvertedForExport(options?: {
    ...
    if (pending.length > 0) {
      ...
      this._totalEntitiesToProcess += pending.length
  ...
  zoomTo(box: AcGeBox2d, margin: number = 1.1) {
    ...
    this.updateCameraDepthRangeFromScene()
    this._isDirty = true
  }

  /**
   * Fits the active camera near/far planes (and elevation) to the current
   * scene Z extent so content far from the default camera plane stays visible.
   *
   * @param sceneBox - Optional scene box already read by the caller; when
   *   omitted the current {@link AcTrScene.box} is used.
   */
  private updateCameraDepthRangeFromScene(sceneBox?: {
    min: { z: number }
    max: { z: number }
    isEmpty(): boolean
  }) {
    const box = sceneBox ?? this._scene.box
    if (!box || box.isEmpty()) {
      return
    }
    this.activeLayoutView.updateCameraDepthRange(box.min.z, box.max.z)
    this._appliedDepthRangeZ = { min: box.min.z, max: box.max.z }
  }

  /**
   * Re-applies the camera depth range when the scene's Z extent grows beyond
   * the last applied range.
   *
   * Covers open flows that frame the camera before the full Z extent exists:
   * the first `zoomTo` runs against a partial scene box, and without this the
   * near/far planes stay stale, permanently clipping later entities with
   * larger Z (they simply never draw — "missing layers" after open).
   *
   * Called from the animate loop at most every
   * {@link DEPTH_RANGE_AUTO_REFRESH_INTERVAL_MS} ms. The check itself is cheap
   * — {@link AcTrScene.box} only recomputes when geometry changed.
   *
   * @param now - Current timestamp (injectable for tests).
   */
  autoRefreshCameraDepthRange(now: number = performance.now()) {
    if (
      now - this._lastDepthRangeAutoCheckAt <
      AcTrView2d.DEPTH_RANGE_AUTO_REFRESH_INTERVAL_MS
    ) {
      return
    }
    this._lastDepthRangeAutoCheckAt = now

    const sceneBox = this._scene.box
    if (!sceneBox || sceneBox.isEmpty()) {
      return
    }
    const applied = this._appliedDepthRangeZ
    if (
      applied &&
      !shouldRefreshCameraDepthRange(applied, {
        min: sceneBox.min.z,
        max: sceneBox.max.z
      })
    ) {
    ...
    this.updateCameraDepthRangeFromScene(sceneBox)
    this._isDirty = true
  }
  ...
  addEntity(entity: AcDbEntity | AcDbEntity[]) {
    ...
    this._totalEntitiesToProcess += entities.length
  ...
  private animate = () => {
    ...
    this.autoRefreshCameraDepthRange()
  ...
  private async batchConvert(
    ...
    for (let i = 0; i < entities.length; ++i) {
      ...
      try {
        ...
        // Spanning entities split into several metas, each rebased around its own
        // run origin; the spatial index still registers one entry per entity,
        // backed by the union of the run boxes.
        const directMetas = tryBuildDirectEntityMetas(entity, this._renderer)
        if (directMetas) {
          let added = false
          try {
            ...
            for (const directMeta of directMetas) {
              const appended = this._scene.addDirectEntity(
                ...
                registerSpatialIndex ? unionBox : null
              )
              if (appended) {
                added = true
                // The union box is registered by the first successfully
                // appended meta; later metas must not re-register the id.
                registerSpatialIndex = false
              }
            }
            if (added) {
  ...
  private decreaseNumOfEntitiesToProcess() {
    ...
    if (this._numOfEntitiesToProcess < 0) {
      ...
      // !stillLoading).
      this._isDirty = true
      // Final geometry just landed — grow the camera depth range if it was
      // last fitted to a partial scene (e.g. Saved-mode open framing).
      this.updateCameraDepthRangeFromScene()
    }
  }
}
