...
export class AcDbDxfFiler {
  ...
  peekItem(): AcDbTypedValue | undefined {
    ...
    return pair ? (pair as unknown as AcDbTypedValue) : undefined
  }
  ...
  readItem(): AcDbTypedValue | undefined {
    ...
    this._status = AcDbDxfFilerStatus.Ok
    // The pair already carries `code`/`value`; returning it directly avoids
    // allocating a second wrapper object per pair on this hot path. Consumers
    // only read the returned item, and pushBackItem detects pairs by their
    // `type` field.
    return pair as unknown as AcDbTypedValue
  }
