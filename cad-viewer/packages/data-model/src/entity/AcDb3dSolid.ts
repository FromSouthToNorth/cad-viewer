...
function pointsFromWireframe(wireframe: Float64Array): AcGePoint3d[] {
  const points: AcGePoint3d[] = []
...
export class AcDb3dSolid extends AcDbEntity {
  ...
  private _wireframe: Float64Array = new Float64Array(0)
  ...
  transformBy(matrix: AcGeMatrix3d) {
    ...
    if (this._wireframe.length > 0) {
      const transformed = new Float64Array(this._wireframe.length)
  ...
  subWorldDraw(renderer: AcGiRenderer) {
    ...
    // Float64Array keeps WCS double precision until the renderer rebases the
    // vertices to a local origin (float32 would quantize world-scale
    // coordinates before any rebase could recover them).
    const buffer = new Float64Array(corners.length * 3)
